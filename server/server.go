...
func (m *Server) ListenTarget(hash uint32, c Connection) {
	...
	for {
		...
		if err == nil {
			...
			//log.Info("Writing to client ", c.Client.IP.To4().String())
			continue
		}
...
func (m *Server) HandleClient(hash uint32, from *net.UDPAddr, to *net.UDPAddr, data []byte) error {
	if hash != 0 {
		...
		//log.Info("Writing to target: ", to.IP.To4().String())
	...
	for {
		...
		if err == nil {
			if m.PassUpstream != nil {
				...
				case upstream.HT_REMOTE_IP:
					hash = binary.BigEndian.Uint32(addr.IP.To4())
					break

				case upstream.HT_REMOTE_PORT:
				...
				log.Error("No static nor dynamic upstream, this shouldn't happen")
				break
			}

			//log.Info("Handling client. IP: ", addr.IP.To4().String(), " Hash: ", fmt.Sprintf("%X", hash))
